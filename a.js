	...
	function (t, e, i) {
		...
		class n extends s {
			...
			simpleDraw(t) {
				...
				if (this.isLoad) {
					...
					i.drawImage(this.image, o, h, s, n);
				}
				else {
					i.beginPath(),
		...
		class p extends d {
			constructor(t, e, i) {
				...
				// Cấu hình context cho chất lượng cao
				this.ctx.imageSmoothingEnabled = true;
				this.ctx.imageSmoothingQuality = 'high';
				// Scale context theo devicePixelRatio
				this.setupHighDPI();
			}
			setupHighDPI() {
		...
		class f extends u {
			...
			resizeCanvas() {
				...
				console.log('Canvas resize:', e, 'x', i, 'DPR:', devicePixelRatio);
				
				// Set actual canvas size với high DPI
				this.canvas.width = e * devicePixelRatio;
				this.canvas.height = i * devicePixelRatio;
				
				// Set CSS size để hiển thị
			...
			update() {
				this.resizeCanvas();
				// Reset và setup lại high DPI sau resize
				const render = this.app.getRender();
				if (render && render.ctx) {
					const devicePixelRatio = window.devicePixelRatio || 1;
					render.ctx.setTransform(1, 0, 0, 1, 0, 0);
					if (devicePixelRatio > 1) {
						render.ctx.scale(devicePixelRatio, devicePixelRatio);
					}
					render.ctx.imageSmoothingEnabled = true;
					render.ctx.imageSmoothingQuality = 'high';
				}
				render.update();
			}
		}
		...
		document.addEventListener("DOMContentLoaded", function () {
			// Khởi tạo âm thanh lật sách
			const flipSound = new PageFlipSound();
			let currentSoundType = 'normal'; // Khởi tạo sớm với giá trị mặc định
			...
			document.querySelector(".btn-book-next").addEventListener("click", () => {
				flipSound.playFlipSound(currentSoundType);
				h.flipNext();
			}),
				document.querySelector(".stf__canvas").addEventListener("click", () => {
					flipSound.playFlipSound(currentSoundType);
					h.flipPrev();
				}),
				t.on("flip", (e) => {
			...
			if (soundToggle) {
				soundToggle.addEventListener('click', () => {
					const isEnabled = flipSound.toggle();
					soundToggle.textContent = isEnabled ? '🔊 Âm thanh: BẬT' : '🔇 Âm thanh: TẮT';
					soundToggle.className = isEnabled ? 'btn btn-sm btn-success' : 'btn btn-sm btn-secondary';
				});
			}
			...
			// Preload âm thanh khi trang đã load
			setTimeout(() => {
				flipSound.preload();
			}, 1000);
		});
